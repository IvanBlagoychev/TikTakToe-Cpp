...
bool TryParseInput(string value)      //checking if the input can be parsed to integer and if it is valid (1-9)
{
	try
	{
...
void PrintMatrix(char matrix[3][3])
{
	for (int row = 0; row < 3; row++)
	{
		...
		cout << endl;
		if (row != 2)
			cout << " " << string(21, '-');     // string(21, '-') -- repeat '-' 21 times
		cout << endl;
	}
}
...
bool checkNamesAnswer(char answer, bool nameKeaper)
{
	...
	{
	case 'Y': nameKeaper = true; break;
	case 'N': nameKeaper = false; break;
	default: nameKeaper = false; break;
	}
	return nameKeaper;
}
...
top:
	...
	do
	{
		...
		cin >> InputPosition;     //Take the input as a string at first

		//Transform the input to integer and check if it is valid;

		desiredPosition = stoi(CheckInputValidation(InputPosition));      //desiredPosition = integer from inputPosition string
